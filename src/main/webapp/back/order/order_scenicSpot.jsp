...
<div class="pd-20">
    ...
    <table class="table table-border table-bordered table-bg">
        <thead>
        <tr>
            <th scope="col" colspan="9">景点订单列表</th>
        </tr>
        <tr class="text-c">
            ...
            <th width="150">旅客数量</th>
            <th width="150">总价(元)</th>
            <th width="100">操作</th>
        </tr>
        </thead>
        <tbody>
        <c:forEach items="${pageInfo.list}" var="tOrder">

            <tr class="text-c">
                <td><input type="checkbox" value="${tOrder.id}" name="" class="delBox" ></td>
                <td>${tOrder.id}</td>
                <td>${tOrder.orderid}</td>
                <td>${tOrder.title}</td>
                <td>${tOrder.scenic}</td>
                <td>
                    <fmt:formatDate value="${tOrder.departuretime}" pattern="yyyy-MM-dd"></fmt:formatDate>
                </td>
                <td>${tOrder.total}</td>
                <td>${tOrder.money}</td>
                <td class="td-manage">
                    <a title="删除" href="javascript:;" onclick="admin_del(this,${tOrder.id})"  class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
            </tr>
        </c:forEach>
        </tbody>
    </table>
    ...
    function batchDel() {
        ...
        if (checkBox.length==0){//未选择选框
            ...
            layer.confirm('确认要删除吗？', function (index) {
                ...
                $.ajax({
                    type:"get",
                    url:"order/batchDelScenicSpotOrder",
                    data:"ids="+ids,
                    success:function (result) {
                        if (result.flag){//删除成功
                            layer.msg('已删除',{icon:6,time:1000});
